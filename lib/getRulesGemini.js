...
export async function getRules(categoryNo, questionNo) {
    ...
    const rules = await fetchRules(questionObj);
    // console.log('rules: ', rules);
    return rules;
}

async function fetchRules(questionObj) {
    ...
    const rulesResponse = (await getChoice(questionObj, ruleCategories))?.trim().toLowerCase();
...
export async function getChoice(questionObj, ruleCategories) {

    const response = await ai.models.generateContent({
        // model: "gemini-2.5-flash",
        model: "gemini-2.5-flash-lite-preview-06-17",
        contents: [
            `From the following Question, Answer, and Link, determine the most suitable rule catgories for the question, answer and link and RESPOND ONLY WITH THE INDEXES SEPARATED BY COMMA if it matches, respond only with 'none', if it doesn't match with any rule category. Question: ${questionObj.question}\nAnswer: ${questionObj.answer}\nLink: ${questionObj.link || 'N/A'}\nRule Categories: ${ruleCategories.map((cat, index) => `${index + 1}. ${cat}`).join(', ')}`,
        ],
        config: {
